...
export function getCatalogCategoryUrl(locale: SupportedLocale, categorySlug: string): string {
	const pathname =
		locale === 'en' ? `/en/categories/${categorySlug}` : `/categories/${categorySlug}`;
	return new URL(pathname, `${shopUrl.replace(/\/+$/u, '')}/`).toString();
}
