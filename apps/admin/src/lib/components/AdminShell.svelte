<script lang="ts">
	...
	const navItems: NavItem[] = [
		{ href: '/', icon: LayoutDashboard, label: '儀表板', match: (pathname) => pathname === '/' },
		{
			href: '/orders',
			icon: ClipboardList,
			label: '訂單',
	...
	const activeItem = $derived(navItems.find((item) => item.match(pathname)) ?? navItems[0]!);
	...
	>
		...
		<div class="border-t border-border pt-3">
			...
			>
				...
				<form
					method="POST"
					action={resolve('/logout' as Pathname)}
				>
					<button
						type="submit"
